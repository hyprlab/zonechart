...
(async function () {
  ...
  const [origins, chartResp, zip3, statesTopo] = await Promise.all([
    ...
    fetch("/api/chart"),
    fetch("/static/geo/zip3.geojson").then(r => r.json()),
    fetch("/static/geo/states-10m.json").then(r => r.json()),
  ]);
  if (!chartResp.ok) {
    document.querySelector(".layout").innerHTML =
      `<div class="card" style="grid-column:1/-1;text-align:center;padding:48px 24px">
         ...
         <a href="/admin">the admin page</a> and run a refresh to download
  ...
  async function setOrigin(raw) {
    ...
    const r = await fetch(`/api/chart?origin=${raw}`);
    if (!r.ok) {
